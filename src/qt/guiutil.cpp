...
#include <qt/angcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new ANGCOINAddressEntryValidator(parent));
    widget->setCheckValidator(new ANGCOINAddressCheckValidator(parent));
}

bool parseANGCOINURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no angcoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!ANGCOINUnits::parse(ANGCOINUnits::ANG, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseANGCOINURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseANGCOINURI(uriInstance, out);
}

QString formatANGCOINURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("angcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ANGCOINUnits::format(ANGCOINUnits::ANG, info.amount, false, ANGCOINUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openANGCOINConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", ANGCOIN_CONF_FILENAME));
    ...
    /* Open angcoin.conf with the associated application */
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("angcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a angcoin.desktop file to the autostart directory:
