...
ClientModel is used by the main application `angcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a angcoin:... link from a browser).
...
* UI elements like ANGCOINAmountField, which inherit from QWidget.
* `angcoinstrings.cpp`: automatically generated
* `angcoinunits.(h/cpp)`: ANG / mANG / etc handling
* `callback.h`
...
* don't change `local/angcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
